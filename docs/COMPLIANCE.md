...
| Unified mod controls | Live local configuration objects from exact reviewed versions of installed SkyHanni, Skyblocker, Firmament, and BabyZombieAddons; physical QCA clicks/drags | Provider selection, native setting changes, native HUD position/scale changes | None; writes only local client configuration through each provider's save path |
...
The separate unified-settings adapter does use version-locked reflection to reach the reviewed mods' live client configuration because not all four expose a public cross-mod settings API. It does not use that access for prices, gameplay state, network traffic, hidden server data, or automation. It exposes only values with a validated local read/write contract, calls the provider's own save mechanism, and fails closed when the exact reviewed version or expected structure is absent.
