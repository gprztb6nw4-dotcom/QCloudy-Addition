...
| Middle-click menu conversion | Explicit physical mouse input in an already-open menu | Equivalent middle-click menu activation | Corresponding container action only; middle-click conversion is default-off and defaults to left-click only |
...
QCA contains no Hypixel Mod API, Hypixel public API, WebSocket, HTTP client, telemetry, coordinate sharing, remote updater, macro, simulated input, automatic click/movement helper, or block interaction. The only outbound actions are the documented user-triggered `/th` warp command, `/helia` chapter command, container actions for middle-click conversion, and one ordinary server connection when the player clicks `Reconnect`; none run without physical user input. Reconnect has no timer, retry loop, background attempt, or automatic join.
