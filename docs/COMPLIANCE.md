...
| Storage Overlay navigation | Explicit `/acastorage` input or a physical click on a Storage page/overview control | Opens the selected normal server Storage page | Sends `storage`, `enderchest <1-9>`, or `backpack <1-18>` for the selected control |
...
| Middle-click/slot binding | Explicit physical mouse/key input in an already-open menu | Equivalent menu click or inventory swap | Corresponding container action only; middle-click conversion is default-off and defaults to left-click only |
...
- Registered local storage helper: `/acastorage`; it is skipped on a client-command conflict. When the user invokes it, QCA sends the exact payload `storage`, equivalent to the user entering `/storage`.
...
- Storage overlay navigation: a physical click on Ender Chest page 1–9 sends `enderchest <page>`; a physical click on Backpack page 1–18 sends `backpack <page>`; the explicit return-to-overview control sends `storage`. These commands only request the exact page selected by the player.
...
QCA contains no Hypixel Mod API, Hypixel public API, WebSocket, HTTP client, telemetry, coordinate sharing, remote updater, macro, simulated input, automatic click/movement helper, or block interaction. The only outbound actions are the documented user-triggered Storage commands, `/th` warp command, `/helia` chapter command, container actions for middle-click conversion or slot binding, and one ordinary server connection when the player clicks `Reconnect`; none run without physical user input. Reconnect has no timer, retry loop, background attempt, or automatic join.
