...
- Renamed `Storage UI` to `Storage Overlay` and changed its default to off.
...
- All five QCA hotkeys now edit inline on their existing feature-settings page. Keyboard and mouse buttons, including buttons 1–5/side buttons, support modifier chords. `Esc` clears the active row to unbound, and the removed `KeyChordScreen` is absent from the source and release. Runtime paths were added for mouse-bound Open Settings, Chat Peek, slot lock, item lock, and press/release slot binding.
