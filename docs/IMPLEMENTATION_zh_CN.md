...
本文对应 `Beta-2.6.6+26.1.2`，逐项说明每个公开功能的用途、读取的客户端信息、实现方式、应呈现的效果、默认状态，以及是否会产生对外操作。
...
- **用途：**用清晰区域图替代复杂路线网。
- **读取内容：**本地玩家 X/Y/Z、朝向和计分板地点。
- **实现：**`DwarvenMapProjection` 把坐标与地点映射进一张包含12个不规则区域的原创示意图；Y坐标与明确地点只用于区分重叠区域，显示仍为单层。
- **效果：**区域属性色、粗边界、清晰Minecraft字体英文地点名和实时旋转红色玩家箭头。
- **默认/对外：**开启；纯渲染。
