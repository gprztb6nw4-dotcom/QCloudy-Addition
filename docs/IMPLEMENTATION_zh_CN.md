...
本文对应适用于 Minecraft 26.1.2 与 26.2 的 `Beta-2.7.17`，逐项说明每个公开功能的用途、读取的客户端信息、实现方式、应呈现的效果、默认状态，以及是否会产生对外操作。
...
这些已审查提供方构建均面向 Minecraft 26.1.2。QCA 的 26.2 产物仍完整保留自身功能，但在对应 26.2 提供方构建及其原生配置契约通过审核前不会开放这些适配器。
...
`HudLayoutScreen` 把 QCA 当前实际加载的面板与所选外部提供方中已经启用的 HUD 合并显示。拖动/缩放过程只保留临时预览，松开鼠标才写入原生坐标或缩放。已审核适配层刻意排除尚未审查 setter/校验契约的复杂颜色、快捷键和自定义编辑器对象。
