...
本文对应 `Beta-2.6.9+26.1.2`，逐项说明每个公开功能的用途、读取的客户端信息、实现方式、应呈现的效果、默认状态，以及是否会产生对外操作。
...
- **实现：**直接归属本地玩家的水钓或岩浆鱼钩始终优先。`FishingHookResolver` 在本地玩家真实使用钓竿时记录已经存在的鱼钩 ID；随后的 40 tick 内，只接受新加载、归属本地玩家或 owner 为空的候选，明确属于其他玩家的鱼钩会被拒绝。这样可以覆盖 owner 关联缺失的 Hypixel 岩浆鱼钩，同时不会认领任意旧鱼钩。随后 `FishingBiteAlert` 只接受“不可见实体、名称可见、名称精确等于 `!!!`”的 ArmorStand。`FishingBiteSession` 按选中鱼钩实体 ID 去重，同一根鱼钩只允许播放一次，鱼钩消失或出现新 ID 后才重新待命。提供的 MP3 在打包前转换为 `assets/qcloudy_addition/sounds/fishing/ciallo.ogg`，再通过 `sounds.json` 注册。
