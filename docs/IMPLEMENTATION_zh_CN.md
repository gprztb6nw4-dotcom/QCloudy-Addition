...
本文对应 Minecraft 26.1.2 的 `Alpha-2.8.27`，逐项说明每个公开功能的用途、读取的客户端信息、实现方式、应呈现的效果、默认状态，以及是否会产生对外操作。当前 Alpha 只构建 26.1.2。
...
- **读取内容：**客户端收到的精确 Power Orb 消失聊天行；本地玩家使用的精确 `WARNING_FLARE`、`ALERT_FLARE` 或 `SOS_FLARE` 物品 ID；收到的成功 Flare 放置音效（`entity.firework_rocket.launch`、音调 1、音量 3）。
- **实现：**Power Orb 去除格式代码后按四个名称整行完全匹配。使用 Flare 只建立两秒候选；只有匹配成功放置音效才确认并启动三分钟单调计时。成功放置新 Flare 会静默替换旧计时；换世界、换服务器和断线静默清理。实体卸载、距离和增益范围不参与判断；展示层另有防重复。
