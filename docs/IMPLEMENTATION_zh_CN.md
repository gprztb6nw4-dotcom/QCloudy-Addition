...
本文对应适用于 Minecraft 26.1.2 与 26.2 的 `Beta-2.8.17`，逐项说明每个公开功能的用途、读取的客户端信息、实现方式、应呈现的效果、默认状态，以及是否会产生对外操作。
...
启用对应门控后，QCA 才会探测提供方的实时根对象、受支持的数值访问器和原生 update/save/dirty 能力；必需根对象或保存契约不存在时，只跳过对应适配器。切换任一总开关都会使探测缓存失效，使界面立即按新状态重建。
