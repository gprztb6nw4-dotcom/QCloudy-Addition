...
> **Current version: 0.2.9 Alpha 30 for Minecraft 26.1.2.** Optional third-party settings/HUD adapters use on-demand capability discovery instead of an exact-version whitelist.
...
Put `QCloudy_Addition-0.2.9+26.1.2-Alpha-30.jar` in the instance's `mods` folder. Do not install the `-sources.jar` as the playable mod.
