...
| Deployable 到期提醒 | 精确收到的本人 Power Orb/Flare 消失聊天行 | 中央大字与独立配置的本地音效 | 无 |
| Century Cake 到期追踪 | 精确收到的 48 小时刷新聊天行与本地系统时间 | 本地计时、效果界面、中央/聊天提醒与本地音效 | 只有玩家直接点击带下划线的续效果文字后才执行精确 `/visit northwestcloudy`；绝不自动执行 |
