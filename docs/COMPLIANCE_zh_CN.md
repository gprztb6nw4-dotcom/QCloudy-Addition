...
| 统一模组控制 | 已安装且版本与审查版本完全一致的 SkyHanni、Skyblocker、Firmament、BabyZombieAddons 实时本地配置对象；玩家在 QCA 中的真实点击/拖动 | 提供方选择、原生设置、原生 HUD 位置/缩放变化 | 无；只通过对应提供方保存路径写入本地客户端配置 |
...
独立的统一设置适配器会使用版本锁定反射访问已审查模组的实时客户端配置，因为这四个模组并非都提供公开的跨模组设置 API。该访问不用于价格、玩法状态、网络通信、隐藏服务器数据或自动化。它只开放具有可验证本地读写契约的值，调用提供方自己的保存机制；版本或结构不匹配时直接关闭集成。
