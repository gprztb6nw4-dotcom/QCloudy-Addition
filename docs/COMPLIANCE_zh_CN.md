...
| 统一模组控制 | 已安装 SkyHanni、Skyblocker、Firmament、BabyZombieAddons 中能够识别的实时本地配置能力；玩家在 QCA 中的真实点击/拖动 | 提供方选择、原生设置、原生 HUD 位置/缩放变化 | 无；只通过对应提供方自己的保存/update 路径写入本地客户端配置 |
...
- 可选提供方集成按能力探测，而不是精确版本白名单。第三方设置与 HUD 分别受两个默认关闭的总开关控制：设置开关关闭时不探测或写入提供方设置，HUD 开关关闭时不探测或写入提供方 HUD 位置。玩家选择启用对应集成后，QCA 也只读写已识别的实时字段，并调用已安装提供方自己的保存/update 机制；未知字段和不支持的数值类型会被省略，QCA 不直接编辑其他模组的配置文件。
...
独立的统一设置适配器会使用经过能力检查的反射访问已安装模组的实时客户端配置，因为这四个提供方没有共同且稳定的跨模组设置 API。提供方版本号不再作为白名单。提供方探测必须由玩家主动开启：默认关闭的设置总开关控制配置探测/编辑，另一个默认关闭且相互独立的 HUD 总开关控制实时 HUD 探测/编辑。启用对应总开关后，适配器才验证已识别的配置根对象和原生保存/update 能力，只开放能够独立读取并安全写入的受支持值；未知或已经改变的分支会逐项跳过。如果提供方删除了必须的根对象或保存契约，才只关闭该提供方的适配器。该访问不用于价格、玩法状态、网络通信、隐藏服务器数据或自动化，QCA 也不会直接修改其他模组的配置文件。
