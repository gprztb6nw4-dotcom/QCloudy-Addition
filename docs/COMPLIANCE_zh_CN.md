...
| Storage 覆盖导航 | 玩家主动输入 `/acastorage`，或真实点击存储页/总览控件 | 打开玩家所选的正常服务器 Storage 页面 | 根据所选控件发送 `storage`、`enderchest <1-9>` 或 `backpack <1-18>` |
| Torrhus 快捷命令 | 玩家主动输入本地 `/th` | 无 | 发送精确内容 `warp torrhus` |
| Helia 快捷命令 | 玩家主动输入本地 `/helia` | 无 | 发送精确内容 `chapter torrhus` |
| 中键/槽位绑定 | 玩家在已打开菜单中的真实鼠标或按键输入 | 等效菜单点击或背包交换 | 仅发送对应容器操作；中键替代默认关闭且默认只转换左键 |

## 命令与聊天

- 本地设置命令：`/aca`、`/qca`、`/ca`、`/qc`；若其他客户端命令已占用某个根名称，则跳过该别名。它们只打开 QCA 设置，不发送内容。
- 本地存储入口：`/acastorage`；若发生客户端命令冲突则跳过。玩家主动调用时，QCA 发送精确 payload `storage`，等同玩家输入 `/storage`。
...
- 存储覆盖层导航：玩家真实点击 Ender Chest 第 1–9 页时发送 `enderchest <页码>`；点击 Backpack 第 1–18 页时发送 `backpack <页码>`；主动点击返回总览控件时发送 `storage`。这些命令只请求玩家刚刚选择的页面。
...
QCA 不包含 Hypixel Mod API、Hypixel 公共 API、WebSocket、HTTP 客户端、遥测、坐标共享、远程更新器、宏、模拟输入、自动点击/移动或方块交互。唯一的对外动作是上面明确记录、由玩家触发的存储命令、`/th` 传送命令、`/helia` Chapter 命令、中键转换或槽位绑定容器操作，以及玩家点击“重新连接”后的一次普通服务器连接；它们都不会自行运行。重连没有倒计时、重试循环、后台尝试或自动加入。
