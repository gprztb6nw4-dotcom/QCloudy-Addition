...
## 统一设置与 HUD 控制——Beta 2.8.17

QCA 以功能为中心，统一显示自身以及已安装 SkyHanni、Skyblocker、Firmament、BabyZombieAddons 中能够安全识别的设置。“通用”中有两个独立总开关：“统一设置编辑”控制第三方设置探测/编辑，“统一 HUD 编辑”控制第三方 HUD 探测/编辑；两者均默认关闭，且不影响 QCA 自身内容。提供方版本号不再作为白名单；启用对应总开关后，QCA 才会探测实时配置/保存能力，加入已识别且可写的字段，跳过未来版本中不认识的结构。一级顺序固定为：**通用、地图、物品与菜单、战斗、地牢、Slayer、挖矿、种地、砍树、钓鱼、狩猎、Rift、活动**。Safari 归入狩猎，Garden 归入种地，Crimson Isle/Kuudra 归入战斗。下级组默认收起，一个完全相同的功能只有一张卡片。

共享功能右键二级菜单的第一项用于选择提供方。开启统一卡片时，会启用所选实现，并只关闭其他兼容提供方中完全等价的实现。通过适配器安全检查的布尔、枚举、有边界数值、位置与缩放设置可直接编辑，并经对应模组原生保存路径落盘。启用独立 HUD 总开关后，QCA 的“编辑 HUD”也会显示已启用、已选择提供方的 HUD，松开鼠标时才提交原生位置/缩放。缺少能力或结构未知的内容会被隐藏，QCA 仍可独立运行。
