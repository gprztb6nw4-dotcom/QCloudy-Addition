...
**功能：**一个默认开启的总开关控制全部受支持蛋糕效果。精确收到 `Big Yum! You refresh/gain <加成> for 48 hours!` 后，以绝对时间记录真实世界 48 小时到期点，因此离线时间也继续计算。`/cake` 与 `/centurycakeeffect` 打开仿原版 `/effects` 的本地界面，显示全部 20 个蛋糕头像、加成、品质、状态和剩余时间。单个过期显示中央大字和 `[QC] Century Cake <Effect> Expired! Click Here For Cake Eating`；同一次检查发现的多个过期会合并成一条数量提醒。带下划线的链接只有在玩家点击后才发送精确 `/visit northwestcloudy`。独立本地音效默认开启且为 64%。
