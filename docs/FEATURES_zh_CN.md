...
所有热键都直接在对应功能的二级设置行内捕获，不再打开独立组合键界面。等待输入的行支持键盘键、鼠标 1–5 以及更多 GLFW 侧键，并可同时记录 Ctrl、Shift、Alt、Cmd/Super；按 `Esc`、Backspace 或 Delete 会清空绑定。菜单中键替代默认关闭，首次开启只转换玩家真实左键。
