...
**功能：**开启后，QCA 只检查本地玩家自己已经加载的 Fishing Hook 周围四格。仅当附近存在“实体不可见、名称可见、收到的名称精确等于 `!!!`”的 ArmorStand 时，才播放一次提供的 Ciallo 提示音。持续存在的标记不会每 tick 重播；鱼钩消失后才为下一次抛竿重新待命。功能默认关闭，拥有独立的 0–100% 连续音量滑块，默认 64%。

**安全边界：**检测器只读取已加载客户端实体并播放模组资源包内的 OGG；不收杆、抛竿、点击、转向、移动、切换物品，也不发送数据包、聊天或命令。
