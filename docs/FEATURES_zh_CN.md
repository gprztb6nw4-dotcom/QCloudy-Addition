...
## 统一设置与 HUD 控制——Alpha 2.8.27

QCA 以功能为中心，统一显示自身以及已安装 SkyHanni、Skyblocker、Firmament、BabyZombieAddons、Feesh 中能够安全识别的设置。“通用”中有两个独立总开关：“统一设置编辑”控制第三方设置探测/编辑，“统一 HUD 编辑”控制第三方 HUD 探测/编辑；两者均默认关闭，且不影响 QCA 自身内容。每一次提供方扫描都必须经过第二次确认：首次开启没有有效会话快照的编辑器，以及每一次点击 Refresh，都会先显示对应设置或 HUD 范围的确认窗口。取消首次确认会保持开关关闭；取消 Refresh 会保留当前有效快照；重启后恢复为开启的开关不会静默扫描。确认后才会打开实时进度页。设置页只显示设置数量，HUD 页只显示 HUD 数量；未安装模组不会显示。Refresh 期间继续使用上一份有效结果，直到新结果完成并校验后才替换；扫描运行时 Refresh 不可点击；两个开关都关闭时会取消任务并释放快照。提供方版本号不作为白名单，仍兼容的已识别能力会保留，变化分支会逐项跳过。一级分类按 **通用、地图、物品与菜单、战斗、地牢、Slayer、挖矿、种地、砍树、钓鱼、狩猎、Rift、活动** 排序，但侧栏只显示当前至少拥有一个 QCA 或已发现提供方功能的分类。Safari 归入狩猎，Garden 归入种地，Crimson Isle/Kuudra 归入战斗；钓鱼提示使用“咬钩提示”下级组。下级组默认收起，一个完全相同的功能只有一张卡片。Alpha 2.8.27 只发布 Minecraft 26.1.2 构建。
...
**功能：**精确匹配客户端收到的本人 Radiant、Mana Flux、Overflux、Plasmaflux Power Orb 消失聊天行。Warning、Alert、SOS Flare 使用确认后的本地生命周期：使用精确 Flare 道具后，必须紧接着匹配成功放置音效，才会启动三分钟计时。放置失败、实体卸载、换世界、距离与增益范围都不会制造错误到期提醒。结束时以红色中央大字显示 `<Deployable Name> Despawned!!!`。Power Orb、Flare、中央大字、音效和 0–100% 音量可分别设置；功能与音效默认开启，音量默认 64%。
...
**功能：**一个默认开启的总开关控制全部受支持蛋糕效果。追踪器只接受两种客户端真实消息：首次生效 `Yum! You gain <加成> for 48 hours!`，以及刷新 `Big Yum! You refresh <加成> for 48 hours!`。匹配前会规范化 Hunter Fortune 等数值中附带的私用区属性图标。有效消息会以绝对时间记录真实世界 48 小时到期点，因此离线时间也继续计算。`/cake` 与 `/centurycakeeffect` 打开仿原版 `/effects` 的本地界面，显示全部 20 个蛋糕头像、加成、品质、状态和剩余时间。单个过期显示中央大字和 `[QC] Century Cake <Effect> Expired! Click Here For Cake Eating`；同一次检查发现的多个过期会合并成一条数量提醒。带下划线的链接只有在玩家点击后才发送精确 `/visit northwestcloudy`。独立本地音效默认开启且为 64%。
