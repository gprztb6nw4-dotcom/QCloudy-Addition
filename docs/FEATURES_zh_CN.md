...
共享功能右键二级菜单的第一项用于选择提供方。开启统一卡片时，会启用所选实现，并只关闭其他兼容提供方中完全等价的实现。通过适配器安全检查的布尔、枚举、有边界数值、位置与缩放设置可直接编辑，并经对应模组原生保存路径落盘。QCA 的“编辑 HUD”也会显示已启用、已选择提供方的 HUD，松开鼠标时才提交原生位置/缩放。未安装或未审查版本的提供方会被隐藏，QCA 仍可独立运行。
