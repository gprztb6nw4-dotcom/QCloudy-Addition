...
> **当前通道：Alpha 2.8.22。** 同一套 QCA 自身功能同时构建为 Minecraft 26.1.2 与 26.2 版本。可选第三方设置/HUD 适配改为按需能力探测，不再使用精确版本白名单。
...
根据游戏版本，将 `QCloudy_Addition-Alpha-2.8.22+26.1.2.jar` 或 `QCloudy_Addition-Alpha-2.8.22+26.2.jar` 放入实例 `mods` 文件夹。不要把 `-sources.jar` 当作可运行模组安装。
