...
> **当前通道：Alpha 2.6.17。** 同一套 QCA 自身功能同时构建为 Minecraft 26.1.2 与 26.2 版本。第三方设置/HUD 精确版本适配目前只对 26.1.2 完成审核；26.2 中会安全保持不可用，直到对应提供方构建通过检查。
...
- 八个固定顺序的一级分类：通用、地图、物品与菜单、战斗、挖矿、砍树、钓鱼、狩猎；支持搜索与可折叠功能组，每项功能只有一个归属。
...
根据游戏版本，将 `QCloudy_Addition-Alpha-2.6.17+26.1.2.jar` 或 `QCloudy_Addition-Alpha-2.6.17+26.2.jar` 放入实例 `mods` 文件夹。不要把 `-sources.jar` 当作可运行模组安装。
