...
> **当前通道：Alpha 2.6.14。** 这是统一设置的首次实现，并修复未满级 Ancient Golden Dragon 的等级显示。源码与构建已经过自动验证，但直接设置写入和第三方 HUD 位置仍需在不同提供方配置与 GUI Scale 下进行广泛游戏内回归。
...
将可运行文件 `QCloudy_Addition-Alpha-2.6.14+26.1.2.jar` 放入实例的 `mods` 文件夹。不要把 `-sources.jar` 当作可运行模组安装。
