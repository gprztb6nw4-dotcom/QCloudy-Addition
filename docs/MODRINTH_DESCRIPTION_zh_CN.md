...
> **当前通道：Beta 2.6.6。** Beta 表示该构建已经通过自动测试和归档验证，但仍需要更广泛的实服、材质包、GUI Scale、操作系统和模组组合测试。

## 核心功能

### 地图

- 简洁的单层 **Dwarven Mines 地图**，使用符合区域范围的独立形状和实时玩家箭头。
...
将可运行文件 `QCloudy_Addition-Beta-2.6.6+26.1.2.jar` 放入实例的 `mods` 文件夹。不要把 `-sources.jar` 当作可运行模组安装。
