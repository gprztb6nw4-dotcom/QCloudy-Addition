...
### 6.2 Deployable Expiry Alert

**Purpose:** make it immediately obvious when a temporary combat support deployable has ended.

**What it does:** exactly matches the received player-owned despawn line for Radiant, Mana Flux, Overflux, and Plasmaflux Power Orbs and Warning, Alert, and SOS Flares. It displays `<Deployable Name> Despawned!!!` as a large red center title. The feature defaults on; its independent local sound defaults on at 64% and has a continuous 0–100% slider.
