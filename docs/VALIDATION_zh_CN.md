...
- 五个 QCA 热键全部改为在原有功能二级页中行内编辑；键盘、鼠标 1–5/侧键及修饰键组合均可使用。等待输入时按 `Esc` 会清空为未绑定，旧 `KeyChordScreen` 已从源码和发布包删除。运行时补齐鼠标绑定的打开设置、Chat Peek、槽位锁定、物品锁定和按下/释放槽位绑定路径。
