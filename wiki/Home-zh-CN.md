...
|---|---|---|
| 26.1.2 | 0.155.2+26.1.2 或更高的兼容版本 | `QCloudy_Addition-Beta-2.8.17+26.1.2.jar` |
| 26.2 | 0.154.2+26.2 或更高的兼容版本 | `QCloudy_Addition-Beta-2.8.17+26.2.jar` |
...
“通用”中有两个相互独立的总开关。“统一设置编辑”控制第三方设置的探测与编辑；“统一 HUD 编辑”控制第三方 HUD 的探测、位置与缩放编辑。两者均默认关闭。关闭任一开关只停止对应第三方集成路径，不会关闭 QCA 自身的设置或 HUD。
...
提供方版本号不再作为白名单。启用对应总开关后，每次打开编辑器时，QCA 才会探测已安装提供方中能够识别的配置与保存能力。因此普通版本更新后，结构仍兼容的原有功能可继续编辑；QCA 无法安全理解的新结构会被省略。提供方未安装或必需根对象/保存契约缺失时会安全关闭对应适配器，不影响 QCA 自身功能。
...
Beta 2.8.17 使用 Java 25 为两个 Minecraft 版本构建。当前验证报告会记录本版本准确的自动测试、压缩包、语言与兼容契约检查结果。
